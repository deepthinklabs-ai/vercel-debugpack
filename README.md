...
3. **On your staging site**, press **Ctrl+Shift+;** (or Cmd+Shift+; on Mac) to enable debug mode
...
Add the `<DebugPanel />` component to your root layout. It only renders on preview deployments when debug mode is enabled via **Ctrl+Shift+;** keyboard shortcut (Cmd+Shift+; on Mac).
...
<DebugPanel
  ...
  config={{
    ...
    keyboardShortcutKey: ';', // Key to use with Ctrl+Shift (default: ';')
  }}
/>
```
...
- Debug mode only activates on preview environments (auto-detected or via `previewUrlPattern`) with Ctrl+Shift+; keyboard shortcut (configurable via `keyboardShortcutKey`)
